...
export class NotificationService {
  ...
  async handleStatusChange(
    ...
    try {
      ...
      // Add debug email to recipients
      const enhancedRecipients = addDebugRecipient(recipients);
      ...
      for (let attempts = 1; attempts <= this.maxRetries; attempts++) {
        try {
          const result = await cloudFunction({
            ...
            recipients: enhancedRecipients.to,
            cc: enhancedRecipients.cc,
            emailContent
          });
          ...
          await this.logNotification(
            'STATUS_CHANGE',
            context.prId,
            [...(enhancedRecipients.to || []), ...(enhancedRecipients.cc || [])],
            'SENT',
            {
              prNumber: context.prNumber,
