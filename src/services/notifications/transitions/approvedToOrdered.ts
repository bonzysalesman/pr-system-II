...
/**
 * Handles the transition from APPROVED to ORDERED status
 */
export class ApprovedToOrderedHandler implements StatusTransitionHandler {
  /**
  ...
  async getRecipients(context: NotificationContext): Promise<Recipients> {
    // Primary recipients: procurement team
  ...
  async getEmailContent(context: NotificationContext): Promise<EmailContent> {
    // Generate subject with urgency indicator if needed
    const urgentPrefix = context.isUrgent ? 'URGENT: ' : '';
    const subject = `${urgentPrefix}PR #${context.prNumber} Has Been Ordered`;
    
    // Get requestor and user info
    const requestorInfo = getRequestorInfo(context);
    ...
    const prDetails = {
      ...
      preferredVendor: context.pr?.preferredVendor || 'Not specified',
      vendorName: context.pr?.vendorName || context.pr?.vendor?.name || 'Not specified',
      expectedLandingDate: context.pr?.expectedLandingDate || 'Not specified'
    };
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ${context.isUrgent ? 
        `<div style="background-color: #ff4444; color: white; padding: 10px; text-align: center; font-weight: bold;">
          URGENT REQUEST
        </div>` : 
      ...
      <div style="background-color: #f8f8f8; padding: 20px; border-bottom: 3px solid #e0e0e0;">
        <h2 style="margin: 0; color: #333;">PR #${context.prNumber} Has Been Ordered</h2>
      </div>
      
      <div style="padding: 20px;">
        <p>The purchase request has been ordered by ${changedByUser}.</p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          ...
          <tr>
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">APPROVED → ORDERED</td>
          </tr>
          <tr>
            <th style="text-align: left; padding: 8px; border: 1px solid #ddd; background-color: #f2f2f2;">Expected Delivery</th>
            <td style="padding: 8px; border: 1px solid #ddd;">${prDetails.expectedLandingDate}</td>
          </tr>
          ${context.notes ? `
          <tr>
    ...
    const text = `
${context.isUrgent ? 'URGENT REQUEST' : 'PURCHASE REQUEST UPDATE'}

PR #${context.prNumber} Has Been Ordered

The purchase request has been ordered by ${changedByUser}.
...
Status Change: APPROVED → ORDERED
Expected Delivery: ${prDetails.expectedLandingDate}
