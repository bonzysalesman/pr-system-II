...
import { User as NotificationUser } from '@/types/user';
...
export async function getPR(prId: string): Promise<PRRequest | null> {
  ...
  try {
    ...
    // Use safe accessors for nested objects
    const safeApprovalWorkflow = data.approvalWorkflow || {};
    
    // Construct the PR object with safe defaults for all fields
    const pr: PRRequest = {
      id: docSnap.id,
      ...
      status: (data.status as PRStatus) || PRStatus.DRAFT,
      expenseType: data.expenseType || '',
      estimatedAmount: data.estimatedAmount || 0,
      currency: data.currency || 'LSL',
      totalAmount: data.totalAmount || 0,
      requestor: data.requestor || { id: '', email: '' },
      ...
      lineItems: Array.isArray(data.lineItems) 
        ? data.lineItems.map((item: any): LineItem => ({ ...item })) 
        : [],
      quotes: Array.isArray(data.quotes) ? data.quotes : [],
      attachments: Array.isArray(data.attachments) ? data.attachments : [],
      history: Array.isArray(data.history) 
        ? data.history.map((item: any): HistoryItem => ({
            ...item,
            timestamp: safeTimestampToISO(item.timestamp) || new Date().toISOString(), 
          })) 
        : [],
      approvalWorkflow: { 
        currentApprover: safeApprovalWorkflow.currentApprover || null,
        lastUpdated: safeTimestampToISO(safeApprovalWorkflow.lastUpdated) || new Date().toISOString(),
        approvalHistory: Array.isArray(safeApprovalWorkflow.approvalHistory) 
          ? safeApprovalWorkflow.approvalHistory.map((item: any) => ({
              ...item,
              timestamp: safeTimestampToISO(item.timestamp) || new Date().toISOString()
          }))
          : [],
...
export async function updatePRStatus(
  ...
  user?: UserReference 
    ...
    try {
        ...
        try {
            // Convert UserReference to the User type expected by notification service
            ...
            await notificationService.handleStatusChange(
                prId,
                currentData.status || null,
                status,
                notificationUser,
                { notes: notes }
            );
            logger.info(`Successfully sent notification for PR ${prId} status change to ${status}`);
        } catch (notificationError) {
            // Log but don't fail the transaction if notification fails
            logger.error(`Failed to send notification for PR ${prId} status change:`, notificationError);
        }
...
export async function getUserPRs(
    ...
    try {
        ...
        querySnapshot.forEach((docSnapshot) => {
            ...
            // Build a properly typed PRRequest object
            const pr: PRRequest = {
                id: docSnapshot.id,
                ...
                statusHistory: Array.isArray(data.statusHistory) 
                    ? data.statusHistory.map((item: any): StatusHistoryItem => ({
                        ...item,
                        timestamp: safeTimestampToISO(item.timestamp) || new Date().toISOString(),
                    }))
                    : [],
