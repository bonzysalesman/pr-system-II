...
import { ApproverDiagnosticTool } from '../diagnostic/ApproverDiagnosticTool';
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const approverPermissionLevels = [1, 2, 6]; // Admin, Approver, Procurement
    setIsApprover(!!user && approverPermissionLevels.includes(user.permissionLevel || 0));
  }, [user]);
    ...
    <Box sx={{ p: 3 }}>
      <Grid container spacing={3}>
        ...
        {isApprover && (
          <Grid item xs={12}>
            <ApproverDashboard />
            {/* Diagnostic tool for troubleshooting approver PR visibility */}
            <ApproverDiagnosticTool />
            <Divider sx={{ my: 3 }} />
          </Grid>
        )}
